...
// player_io_answer arrives in M2.

#include "greatest.h"
#include "player_io.h"
#include "game.h"
#include "pending.h"   // pending_flow — the M2 routing source of truth
...
TEST ack_does_not_pop_a_decision(void) {
    ...
    // Use FLOW_DISMISS_LAST: in M2 the router keys off the global pending_flow
...
// Filling to capacity succeeds; the next enqueue is dropped (returns NULL) and
// does not corrupt the queue. Then draining all yields exactly CAP requests.
TEST capacity_bound_drops_overflow(void) {
    Game *g = fresh_game(); ASSERT(g);
    for (int i = 0; i < PLAYER_IO_QUEUE_CAP; i++)
        ASSERT(player_io_enqueue_message(g, "x", ""));
    // Queue is full now; one more must be refused (NULL), not overflow.
    ASSERT_EQ(NULL, player_io_enqueue_message(g, "overflow", ""));
    // Exactly CAP requests drain.
    int drained = 0;
    while (!player_io_idle(g)) { player_io_ack(g); drained++; }
...
// M3: player_io_message enqueues a REQ_MESSAGE; drain_messages acks all leading
...
// M3: raise_decision evicts a stale queued decision (one live decision invariant)
...
// M4: raise_view enqueues a REQ_VIEW carrying the replace flag + context, and
...
// M4: raise_view evicts a stale queued view (one-live-view invariant) but keeps
