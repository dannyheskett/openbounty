...
//
// This binary links libobengine.a + engine/host_noop.c + the autoplay and
// demo objects, against libc/libm/pthread only -- no raylib and nothing from
// src/. If the link succeeds, the engine, autoplay and demo layers are
// self-contained; any shell dependency leakage fails the link.
//
// Built by the $(LIBTEST_STAMP) rule, which `make all` depends on, so the
// boundary is checked on every build.
