...
void screen_end_game_open(bool won, const char *body) {
    ...
    // M4: the VIEW_WIN/VIEW_LOSE presentation is raised through the player-IO
...
void screen_end_game_draw(const Game *g, const Sprites *s) {
    ...
    // glued into the corner. The body is pre-wrapped to exactly 18 cols ×
    ...
    const int MARGIN_T = 2;   // top inset (fits: 21 rows × 8 = 168 ≤ 170 − 2)
