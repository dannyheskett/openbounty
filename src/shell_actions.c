...
void shell_dispatch_action(ShellCtx *ctx, const InputState *in) {
    ...
    case INPUT_ACTION_FLY:
        // allowed only from RIDE; every army stack must contain a
        // flying troop with skill_level >= 2 (player_can_fly). Silent
        // on failure.
    ...
    case INPUT_ACTION_LAND: {
        // from FLY, land only on plain grass (terrain=GRASS, no
        // interactive, not blocks_foot).
        if (g->character.mount != MOUNT_FLY) break;
        const Tile *cur = MapGetTile(m, g->position.x, g->position.y);
        if (cur &&
