// src/legacy/overlay.c -- FROZEN.
//
// The overlay exactly as the DOS original draws it: the rounded bottom box,
// the fixed-size menu and options panels, the '>' cursor, the 30-column wrap.
// Its behaviour is the spec, so nothing here changes to serve a modern need --
// modern UI work goes in src/modern/overlay.c. tests/unit/test_legacy_freeze.c
// pins the numbers these draws read.
//
...
void legacy_overlay_draw_dialog(void)          { draw_dialog_ex(DLG_MODE_BOTTOM); }
void legacy_overlay_draw_dialog_centered(void) { draw_dialog_ex(DLG_MODE_CENTERED_MODAL); }
...
static void draw_dialog_ex(DialogMode mode) {
    ...
    if (header_rows) {
        const char *hp = hdr;
        char hline[128];
        // One drawn line per authored newline.
...
void legacy_overlay_draw_town(const Game *g, const Sprites *s) {
    ...
    // Sized from the content rect plus a sidebar, so it lines up under the
    // backdrop instead of spanning the whole pane. 288 wide at x=16 -- what it
    // has always been.
...
void legacy_overlay_draw_options(const Game *g) {
    ...
    int w = cols * GW + 2 * pad;
    int h = rows * GH + 2 * pad;
    const int kb_cols = 1;   // one column, always: the panel is a fixed size
...
void legacy_overlay_draw_controls(const Game *g) {
    ...
    int pad = 3;
    int cols = 18;
    int rows = vis + 1;                           // title plus the settings
    int w = cols * GW + 2 * pad;
