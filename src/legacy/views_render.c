// src/legacy/views_render.c -- FROZEN.
//
// The detail views exactly as the DOS original draws them: the character card
// with no ring, the '>' cursor on the spell and gate lists, newline-only text
// blocks. Its behaviour is the spec, so nothing here changes to serve a modern
// need -- modern UI work goes in src/modern/views_render.c.
//
...
static void draw_view_panel(void) {
    ...
    legacy_window_frame(VIEW_X, VIEW_Y, VIEW_W, VIEW_H, PAL_CLR(DRED));
}
...
static void draw_character(const Game *g, const Sprites *s) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        ui_blit(tex, inv_x + col * item_w, inv_y + row * item_h,
                item_w, item_h);
        legacy_panel_frame(inv_x + col * item_w, inv_y + row * item_h, item_w, item_h);
    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        ui_blit(tex, map_x + col * item_w, inv_y + row * item_h,
                item_w, item_h);
        legacy_panel_frame(map_x + col * item_w, inv_y + row * item_h, item_w, item_h);
    }
}
...
static void draw_army(const Game *g, const Sprites *s) {
    ...
    int vw = FULL_VIEW_W;
    gfx_rect(vx, VIEW_Y, vw, VIEW_H, PAL_CLR(DGREY));
    legacy_window_frame(vx, VIEW_Y, vw, VIEW_H, PAL_CLR(DRED));
...
static int draw_text_block(const char *text, int x, int y, int w, Color c) {
    (void)w;   // the legacy block breaks on authored newlines only
    if (!text) return y;
    const char *p = text;
    char line[96];
    int ly = y;
...
static void draw_contract(const Game *g, const Sprites *s) {
    ...
    legacy_panel_frame(tx, ty, face_w, face_h);
    gfx_rect_lines(tx - CL_UI, ty - CL_UI,
...
static void draw_puzzle(const Game *g, const Sprites *s) {
    ...
    // legacy this is exactly the content rect, at 16,22, as before.
    {
        ...
        legacy_window_frame(px, py, pw, ph, PAL_CLR(DRED));
    }
