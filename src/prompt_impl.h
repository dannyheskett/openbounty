...
typedef struct {
    ...
    // Modern numeric and A/B prompts: one row per choice, named by whoever
    // opened the prompt (prompt_set_choices), and `lead`, the words shown
    // with them; each choice is a label and the answer it gives (1 =
    // PROMPT_RESULT_1).
    const char    *lead;
    int            choice_n;
