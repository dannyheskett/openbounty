...
bool views_spells_update(void) {
    ...
    if (harness_key_pressed(KEY_LEFT))  spell_state.column = 0;
    if (harness_key_pressed(KEY_RIGHT)) spell_state.column = 1;
    ...
    for (int i = 0; i < 7; i++) {
        if (harness_key_pressed(KEY_A + i)) {
...
bool views_menu_update(const MenuCallbacks *cbs, void *userdata) {
    ...
    if (harness_key_pressed(KEY_UP) || harness_key_pressed(KEY_W) || harness_key_pressed(KEY_KP_8)) {
        f->cursor = (f->cursor - 1 + n) % n;
        return true;
    }
    if (harness_key_pressed(KEY_DOWN) || harness_key_pressed(KEY_S) || harness_key_pressed(KEY_KP_2)) {
...
bool views_town_update(Game *g) {
    ...
    if (town.info_active) {
        ...
        if (harness_key_pressed(KEY_ESCAPE) || harness_key_pressed(KEY_ENTER) ||
            harness_key_pressed(KEY_KP_ENTER) || harness_key_pressed(KEY_SPACE)) {
            town.info_active = false;
            return true;
        }
        // Letters A-E also dismiss so the player can chain actions.
        for (int k = KEY_A; k <= KEY_E; k++) {
            if (harness_key_pressed(k)) { town.info_active = false; return true; }
        }
        return true;
    }
    ...
    if (harness_key_pressed(KEY_A)) { town.cursor = TOWN_ROW_CONTRACT; town_do_row(g, TOWN_ROW_CONTRACT); return true; }
    if (harness_key_pressed(KEY_B)) { town.cursor = TOWN_ROW_BOAT;     town_do_row(g, TOWN_ROW_BOAT);     return true; }
    if (harness_key_pressed(KEY_C)) { town.cursor = TOWN_ROW_INFO;     town_do_row(g, TOWN_ROW_INFO);     return true; }
    if (harness_key_pressed(KEY_D)) { town.cursor = TOWN_ROW_SPELL;    town_do_row(g, TOWN_ROW_SPELL);    return true; }
    if (harness_key_pressed(KEY_E)) { town.cursor = TOWN_ROW_SIEGE;    town_do_row(g, TOWN_ROW_SIEGE);    return true; }
    return false;
}
