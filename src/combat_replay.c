// src/combat_replay.c — visible-mode combat animator (WS-7).
//
// Draws an autoplay fight at game pace by ANIMATING the per-turn CombatTurnRecord
...
#include "resources.h"
#include "tables.h"
#include "shell_autoplay.h"  // ShellCtx
...
// combat_present is static in combat_loop.c; re-declare a thin local presenter
// using the public render primitives so the animator stays self-contained. It
// mirrors combat_loop.c's combat_present (field draw + scale-to-window blit).
// Provided by combat_loop.c and exposed for the replay (see combat_loop.c).
...
// the same target the resolution uses (autoplay/autoplay.c builds the identical
// one). Returns false if there is no pending combat flow.
...
// Pace one beat: present the current board for ~one SYN tick (0.15s, scaled by
// the autoplay step delay so the demo pace also speeds/slows fights),
...
CombatReplayStatus RenderCombatRecord(void *shell_ctx, CombatMode mode,
    ...
    combat_current_rendered = &c;   // gameplay-test introspection parity
    ...
    // Beat length scales off the autoplay step delay. step_delay 0 (the default,
    // "play back as fast as possible") => beat 0 => one rendered frame per action
    // (full machine frame rate, no wall-clock throttle). A non-zero step_delay
    // slows fights to a watchable cadence proportional to the adventure pacing.
    double beat = shell_autoplay_step_delay();
    ...
    // awards). Held for the read-dwell beat, then dismissed (the autoplay
    // executor has no human keypress to ack it). Full-speed test replay has a
    ...
    if (rv == COMBAT_REPLAY_OK && rec->result == 1) {
        ...
        double dwell = shell_autoplay_read_dwell();
        if (dwell > 0.0) {
