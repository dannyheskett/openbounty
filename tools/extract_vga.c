// VGA graphics-file parser + 8bpp renderer.
//
// Each `*.256` graphics file in the CC archive is a multi-image atlas:
//   u16 LE   image count
//   per-image directory (image_count x 4 bytes):
...
// after the pixel data (offset + 4 + w*h). detect_real_bg sanity-
...
// Tie-break for "most common" is FIRST-SEEN wins.
