...
//
// Inputs (read from the input directory, default legacy/bin/):
...
//   or:   make extract-pack     (regenerates assets/kings-bounty/)
...
// ---- entry point -----------------------------------------------------
//
// extract_run is the library-style entry point behind the game binary's
// `--extract` flag. It writes a loose asset tree under `out_dir`; the
// caller zips it into a .openbounty pack (see PACK-FORMAT.md).
...
// ---- Stage (3): palette ---------------------------------------------
//
// MCGA.DRV @ 0x032D, 768 bytes (256 x 6-bit RGB). The engine expands
...
// ---- Stage (4): font ------------------------------------------------
//
// KB.CH = 1024 bytes. 128 glyphs x 8 rows x 1 byte (1bpp MSB-first).
// Output: 1024x8 RGBA PNG. White-opaque on transparent so bfont.c can
...
// ---- Stage (5): maps ------------------------------------------------
//
// LAND.ORG: 16384 bytes = 4 x 64x64 continents in KB Y-up storage. We
...
// header + 64 rows.
//
// TILE_MAP is baked in below as a static array; the tile_codes inverse
// is built from game.json at runtime via cJSON.

#include "cJSON.h"

typedef struct { unsigned char b; const char *art; } TileMapEntry;

// We ONLY need the byte->art
...
static const TileMapEntry TILE_MAP[] = {
    ...
    { 0x20, "water" },
    // Forest: anchor at 0x2D, edges 0x21..0x2C.
...
static int load_art_to_char(const char *game_json_path,
    ...
    if (cJSON_IsObject(tc)) {
        ...
        cJSON_ArrayForEach(child, tc) {
            ...
            // First-seen wins: multiple chars can map to the same art
            // (e.g. several grass variants), so taking the FIRST char
...
//
//   freq palette   88 x u16 LE   (Hz; index 0 = silence/marker)
//   delay palette  16 x u16 LE   (ms; first 8 = power-of-two-ish)
//   tune ptr table 10 x u16 LE   (offsets relative to DATA_SEGMENT 0x15690)
