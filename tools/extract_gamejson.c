...
static cJSON *emit_classes(void) {
    ...
    for (int c = 0; c < 4; c++) {
        ...
        // Starting gold and starting army for this class.
...
static cJSON *emit_towns_top(void) {
    ...
    for (int i = 0; i < 26; i++) {
        ...
        // Mapping is implicit in OpenKB, which ports it as town N -> castle N.
        char cslug[96];
        ...
        // start, matching KB.EXE/OpenKB runtime initialization (every
...
// Emitted with id slug + boat coords (which OpenKB also carries).
...
static void try_load_towngate_from_kb(const uint8_t *kb, size_t kb_len) {
    ...
    fprintf(stderr,
        ...
        "falling back to the OpenKB transcription (entry 8 may be wrong)\n");
}
