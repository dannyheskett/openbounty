...
//
// M1 scope: the queue mechanics (enqueue/front/idle/ack, and answer = pop) as a
// flat FIFO inside Game. Additive and not yet authoritative — the engine's
// existing flow/dialog/view callbacks still drive behavior; this queue is
// populated alongside and exercised by unit tests. M2 wires player_io_answer to
// the flow_apply_* cores; M2..M5 flip each channel onto the queue and delete the
// old paths.
...
#include "pending.h"       // the mirrored decision scratch (source of truth in M2)
...
#ifdef OB_TRACE_PRES
static const char *ob_role_name(ReqRole r) {
    switch (r) {
        case REQ_NONE:     return "NONE";
        case REQ_DECISION: return "DECISION";
        case REQ_MESSAGE:  return "MESSAGE";
        case REQ_VIEW:     return "VIEW";
        default:           return "?";
    }
}
#endif
...
static PlayerRequest *enqueue(Game *g, ReqRole role,
    ...
    if (q->count >= PLAYER_IO_QUEUE_CAP) {
        // A full queue is a producer bug (at most a few requests are ever
        // outstanding). Log and drop rather than overflow silently.
        fprintf(stdout, "player_io: enqueue dropped — queue full (cap=%d)\n",
                PLAYER_IO_QUEUE_CAP);
        return NULL;
    }
...
PlayerRequest *player_io_raise_decision(Game *g, PendingFlow flow,
                                        ReqPromptKind kind,
                                        const char *header, const char *body) {
    // In M2/M3 the queue is a MIRROR of the authoritative pending_flow, and the
...
void player_io_answer(Game *g, Map *map, Fog *fog, const Resources *res,
    ...
    OB_TRACE("answer flow=%d front=%s", (int)pending_flow,
             player_io_front(g) ? ob_role_name(player_io_front(g)->role) : "NULL");

    // ROUTE ON THE GLOBAL pending_flow, not the queue front. In M2 pending_flow
    // is still the authoritative source of truth for which decision is live: it
    // is what the emit sites set, what both hosts' step-gates check, and — unlike
    // the per-Game queue — it survives worldsnap restore (the queue is part of
    ...
    // mirror we keep in lockstep by popping it here; it becomes authoritative in
    // M4/M5 once view/message consumption and the pending-state move into Game.
    PendingFlow flow = pending_flow;
    if (flow == FLOW_NONE) { if (out_pres) *out_pres = pres; return; }

    // The decision payload is mirrored in the pending_* scratch in M2 (the emit
...
void player_io_ack(Game *g) {
    ...
    OB_TRACE("ack role=%s view=%d body=\"%.40s\"",
             ob_role_name(r->role), (int)r->view, r->body);
    pop_front(g);
}
...
int player_io_drain_messages(Game *g) {
    ...
    for (;;) {
        ...
        OB_TRACE("drain pop role=%s view=%d body=\"%.40s\"",
                 ob_role_name(r->role), (int)r->view, r->body);
        pop_front(g);
        n++;
    }
