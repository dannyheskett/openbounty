...
typedef struct {
    bool opened_dialog;    // handler produced a dialog (sign-post) -- its text is
                           // in dialog_header/dialog_body; the CALLER (step.c,
                           // which has a Game*) raises it via player_io_message so
                           // the message flows through the uniform queue.
                           // adventure_handle_interact stays Game-free.
    char dialog_header[120];
    char dialog_body[512];
    bool bounce_back;      // caller should revert the move (castle/town gate)
    int  artifact_idx;     // global artifact index to claim, or -1
    bool entered_town;     // hero stepped onto a town tile; step.c opens VIEW_TOWN
    // When entered_town: the town tile's metadata, copied so step.c does not
    // need to re-fetch the tile. town_id is empty when the tile has none.
    char town_id[24];
    int  town_boat_x, town_boat_y;
    bool opened_chest;     // stepped onto a treasure_chest -- step.c rolls loot
    ...
    bool opened_castle;    // stepped onto a castle_gate -- step.c shows visit flow
    char castle_id[24];    // when opened_castle: the castle id from the tile
    bool opened_orb;       // stepped onto orb tile -- step.c triggers reveal + dialog
    bool opened_telecave;  // stepped onto telecave -- step.c teleports to paired cave
    char telecave_id[24];  // when opened_telecave: placement id ("telecave_0" etc.)
    bool opened_navmap;    // stepped onto navmap -- step.c unlocks next continent
    ...
    // flag in g->foes[]. step.c does that lookup; adventure just surfaces
