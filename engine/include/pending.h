...
// them out into a shared TU since the same fields are written by the
// step module and read by the main-loop prompt resolver.
//
...
// these is the responsibility of whichever module opens the prompt; the
// main-loop switch in main.c reads them and clears pending_flow back to
// FLOW_NONE.
...
// starts with no pending decision. (The uniform player-IO refactor moves this
// state into Game; until then, reset it explicitly at boot.)
