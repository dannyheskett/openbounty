...
SaveResult SaveGameRead(const char *path,
    ...
    // reproduced exactly; only non-catalog games carry a raw seed.
    cJSON *jcat = cJSON_GetObjectItem(root, "seed_from_catalog");
    cJSON *jidx = cJSON_GetObjectItem(root, "seed_index");
    if (cJSON_IsTrue(jcat) && cJSON_IsNumber(jidx)) {
        g->seed_from_catalog = true;
        g->seed_index = jidx->valueint & 0xFF;
