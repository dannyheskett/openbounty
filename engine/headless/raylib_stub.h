...
// without libraylib.
//
// Used by the playtest binary (build/openbounty-playtest), which links
// engine logic directly and drives it via function calls — no window,
// no audio, no rendering. The real raylib.h is used by the game binary
// (build/openbounty), which renders normally.
//
...
// game builds, and this stub wins for playtest builds.
...
// in a headless playtest run; if they are, behavior is benign.
...
// Input — never reached in headless playtest (the playtest driver
