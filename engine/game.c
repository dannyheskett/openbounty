...
// besides rank-ups, which are unreachable single-zone — see autoplay's caneghor
// fix). TEMPORARY: leadership_current resets to leadership_base at the next week
...
int GameGateDestinations(const Game *g, GateDestKind kind,
    ...
    if (kind == GATE_DEST_CASTLE) {
        ...
        for (int i = 0; i < cn && n < cap; i++) {
            ...
            const ResCastle *rc = &g->res->castles[i];
            if (strcmp(rc->special.flow, "audience") == 0) continue;
