...
static bool pack_insert(Pack *p, const char *rel, unsigned char *data, size_t size) {
    if (strlen(rel) >= PACK_REL_MAX) {
        fprintf(stdout, "pack: entry path too long: %s\n", rel);
        free(data);
        return false;
    }
...
static void parse_identity(Pack *p) {
    ...
    if (!gj) {
        fprintf(stdout, "pack: missing game.json in %s\n", p->path);
        return;
    }
...
bool pack_zip_dir(const char *src_dir, const char *out_zip) {
    ...
    if (!mz_zip_writer_init_file(&zw, out_zip, 0)) {
        fprintf(stdout, "pack: cannot create %s\n", out_zip);
        return false;
    }
...
bool pack_resolve_arg(const char *arg, char *out, size_t cap) {
    ...
    if (has_sep || has_ext) {
        struct stat st;
        if (stat(arg, &st) != 0) {
            fprintf(stdout, "pack: not found: %s\n", arg);
            return false;
        }
    ...
    fprintf(stdout, "pack: '%s' not found in cwd, user data dir, "
                    "or <exe>/assets\n", arg);
    return false;
}
