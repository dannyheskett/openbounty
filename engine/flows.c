...
#include "tables.h"
#include "ui_host.h"   // open_dialog, prompt_yes_no_open
#include "pending.h"
...
void show_lose_game(const Game *g, const Resources *res) {
    ...
    // M4: raise the LOSE view through the queue (the shell sync pushes it). The
...
void show_win_game(Game *g, const Resources *res) {
    ...
    // M4: raise the WIN view through the queue (the shell sync pushes it).
