...
// See docs/OPENBOUNTY-SPEC.md section 25 (Combat) for design notes. The rendered combat
...
/* exposed for tests */ void combat_seed_rng(Combat *c, const Game *g, CombatMode mode,
    ...
    // Mix in a STABLE per-encounter identity (the foe's placement_id or
    // the castle id, supplied via target->seed_key), falling back to the
    // display name. We deliberately do NOT mix in g->stats.followers_killed
    // (cumulative casualty history): it made a fight's seed depend on the
    // order/outcome of all prior fights, which breaks autoplay planner <->
    // live parity -- the planner re-derives the prefix many times while the
    // live run walks it once, so followers_killed differed at the same
    // fight and the same foe could win at plan time but lose live. Seeding
    // from a stable identity makes a fight a pure function of (seed,
    // identity, mode), so the planner's prediction always matches live.
    const char *key = (target && target->seed_key && target->seed_key[0])
                          ? target->seed_key
...
/* exposed for tests */ void combat_compact(Combat *c) {
    ...
    if (!any_died) return;
    // Rebuild umap from surviving units (cheap: 6x5 grid, 10 units).
    memset(c->umap, 0, sizeof c->umap);
    for (int s = 0; s < COMBAT_SIDES; s++) {
        for (int i = 0; i < COMBAT_SLOTS; i++) {
            CombatUnit *u = &c->units[s][i];
...
// ----- Combat spells ---------------------------------------------------------
//
// Spell IDs match game.json's spell catalog ordering (COMBAT_SPELL_* are now
// defined once in combat.h, shared by engine + shell + autoplay):
...
// The direct-damage bases (fireball / lightning / turn_undead). ONE copy, shared
// by the cast dispatcher below and the prediction query -- no other layer may
// restate 25/10/50.
static int combat_spell_damage_base(int spell_idx) {
    switch (spell_idx) {
        case COMBAT_SPELL_FIREBALL:    return 25;
        case COMBAT_SPELL_LIGHTNING:   return 10;
...
// by the shell picker (src/combat_loop.c) and the autoplay casting policy.
int combat_spell_target_filter(int spell_idx) {
    switch (spell_idx) {
        case COMBAT_SPELL_CLONE:       return PICK_FILTER_FRIENDLY;
        case COMBAT_SPELL_TELEPORT:    return PICK_FILTER_ANY_UNIT;
        case COMBAT_SPELL_FIREBALL:    return PICK_FILTER_ENEMY;
        case COMBAT_SPELL_LIGHTNING:   return PICK_FILTER_ENEMY;
        case COMBAT_SPELL_FREEZE:      return PICK_FILTER_ENEMY;
        case COMBAT_SPELL_RESURRECT:   return PICK_FILTER_FRIENDLY;
        case COMBAT_SPELL_TURN_UNDEAD: return PICK_FILTER_UNDEAD;
        default:                       return PICK_FILTER_ENEMY;
    }
}
...
int combat_cast_spell(Combat *c, int side, int spell_idx,
    ...
    // guard is what keeps AI-vs-AI combat (combat_run_headless, heroes[PLAYER]
    // NULL) and the digest tests inert: a cast there is impossible by construction.
    if (!c || side < 0 || side >= COMBAT_SIDES) return COMBAT_CAST_ILLEGAL;
    Game *gw = c->heroes[side];
    if (!gw) return COMBAT_CAST_ILLEGAL;
    if (!gw->stats.knows_magic) return COMBAT_CAST_ILLEGAL;
    if (c->spells_this_round >= 1) return COMBAT_CAST_ILLEGAL;
    ...
    switch (spell_idx) {
        ...
        case COMBAT_SPELL_TELEPORT:
            spell_teleport(c, t_side, t_slot, dest_x, dest_y);
            casted = 1;
            break;
        ...
        case COMBAT_SPELL_FREEZE:
            if (spell_freeze(c, t_side, t_slot) == 1)
                casted = 1;
            break;
        case COMBAT_SPELL_RESURRECT:
            if (spell_resurrect(c, t_side, t_slot, sp) == 1)
                casted = 1;
            break;
    ...
    if (casted) {
        gw->spells.counts[spell_idx]--;
        c->spells_this_round++;
        return COMBAT_CAST_OK;
    }
    return COMBAT_CAST_NO_EFFECT;
}
...
// distance, prefer cardinal over diagonal when tied"). For a 6x5
...
// Default player callback for combat_run_headless: drive the player side with
// the same enemy AI, preserving the original behavior exactly.
static int combat_default_player_fn(Combat *c, void *ctx) {
    (void)ctx;
    return combat_ai_action(c);
}

CombatResult combat_run_headless(Game *g, CombatMode mode,
                                 const CombatTarget *target,
                                 int cap_rounds) {
    return combat_run_headless_ex(g, mode, target, cap_rounds,
                                  combat_default_player_fn, NULL);
}
...
CombatResult combat_run_headless_ex(Game *g, CombatMode mode,
                                    ...
                                    void *player_ctx) {
    return combat_run_headless_rec(g, mode, target, cap_rounds,
                                   player_fn, player_ctx, NULL);
}

// Append one per-action entry to the record. Classifies the action from
// the observable before/after deltas -- positions and counts are the load-bearing
// truth the animator replays; the MOVE/FLY/MELEE/SHOOT label only picks the draw
// flavor, so an imperfect classification is cosmetic, never a parity risk. No-op
// when rec is NULL or full. `pre_pos` is the acting unit's (x,y) before the
// action; `pre_count[side][slot]` the per-stack counts before.
static void combat_rec_append(CombatTurnRecord *rec, const Combat *c,
                              int act_side, int act_slot,
                              int pre_x, int pre_y,
                              const short pre_count[COMBAT_SIDES][COMBAT_SLOTS]) {
    if (!rec || !rec->entries || rec->count >= rec->cap) return;
    CombatTurnEntry *e = &rec->entries[rec->count];
    memset(e, 0, sizeof *e);
    e->act_side = (unsigned char)act_side;
    e->act_slot = (unsigned char)act_slot;
    e->from_x = (signed char)pre_x;
    e->from_y = (signed char)pre_y;
    e->tgt_side = 0xFF; e->tgt_slot = 0xFF;
    const CombatUnit *au = &c->units[act_side][act_slot];
    bool valid_actor = (au->troop_idx >= 0);
    e->to_x = (signed char)(valid_actor ? au->x : pre_x);
    e->to_y = (signed char)(valid_actor ? au->y : pre_y);
    e->act_count_after = (short)(valid_actor ? au->count : 0);
    // Find an enemy stack whose count dropped this action -- the target.
    int enemy = act_side ^ 1;
    int tslot = -1;
    for (int i = 0; i < COMBAT_SLOTS; i++) {
        if (c->units[enemy][i].count < pre_count[enemy][i]) { tslot = i; break; }
    }
    bool moved = valid_actor && (au->x != pre_x || au->y != pre_y);
    if (tslot >= 0) {
        e->tgt_side = (unsigned char)enemy;
        e->tgt_slot = (unsigned char)tslot;
        e->tgt_count_after = (short)c->units[enemy][tslot].count;
        // Adjacent target => melee, else ranged shot.
        int dx = au->x - c->units[enemy][tslot].x;
        int dy = au->y - c->units[enemy][tslot].y;
        bool adj = (dx >= -1 && dx <= 1 && dy >= -1 && dy <= 1);
        e->kind = (unsigned char)(adj ? CT_MELEE : CT_SHOOT);
    } else if (moved) {
        // Flew if it covered more than one tile in a step; else walked.
        int adx = au->x - pre_x, ady = au->y - pre_y;
        bool far = (adx < -1 || adx > 1 || ady < -1 || ady > 1);
        e->kind = (unsigned char)(far ? CT_FLY : CT_MOVE);
    } else {
        e->kind = (unsigned char)CT_WAIT;
    }
    // Copy the most recent log line (the one this action appended, if any).
    if (c->log_count > 0) {
        int li = (c->log_count - 1) % COMBAT_LOG_LINES;
        snprintf(e->log_line, sizeof e->log_line, "%s", c->log_lines[li]);
    }
    rec->count++;
}
...
CombatResult combat_run_headless_rec(Game *g, CombatMode mode,
    ...
    if (!player_fn) player_fn = combat_default_player_fn;
    if (out_rec) { out_rec->count = 0; out_rec->result = 0;
                   out_rec->ai_stacks = 0; out_rec->ai_hp = 0; }
    Combat c;
    ...
    // NO-PROGRESS CUTOFF: when no damage-relevant state (counts, injuries,
    // ammo, spell charges) changes across 8 rounds' worth of actions, the
    // fight is two sides circling -- positions alone can never start dealing
    // damage after that long (the board diameter is 6), so resolve it as the
    // timeout would, without walking the remaining rounds. Positions are
    // deliberately excluded from the fingerprint: pure movement IS the stall.
    long stall_fp = -1; int stall_n = 0;
    const int stall_cap = 8 * 64;
    while (c.result == 0 && actions < max_actions) {
        {
            long fp = 0;
            for (int s2 = 0; s2 < COMBAT_SIDES; s2++)
                for (int i2 = 0; i2 < COMBAT_SLOTS; i2++) {
                    const CombatUnit *u2 = &c.units[s2][i2];
                    fp = fp * 31 + u2->count;
                    fp = fp * 31 + u2->injury;
                    fp = fp * 31 + u2->shots;
                }
        ...
        // Capture pre-action state for the replay record before dispatching.
        int rec_side = c.side, rec_slot = c.unit_id;
        int rec_px = 0, rec_py = 0;
        short pre_count[COMBAT_SIDES][COMBAT_SLOTS];
        // Per-action capture is needed only when actually recording entries. A
        // metrics-only caller (out_rec with entries=NULL) skips it and pays no
        // hot-loop cost; it still gets the end-state ai_stacks/ai_hp below.
        if (out_rec && out_rec->entries) {
            if (rec_slot >= 0 && rec_slot < COMBAT_SLOTS) {
                rec_px = c.units[rec_side][rec_slot].x;
                rec_py = c.units[rec_side][rec_slot].y;
            }
            for (int s = 0; s < COMBAT_SIDES; s++)
                for (int i = 0; i < COMBAT_SLOTS; i++)
                    pre_count[s][i] = (short)c.units[s][i].count;
        }
        // Player side uses the supplied policy; AI side uses combat_ai_action.
        // Both share the same "nonzero = consumed the turn" contract.
        int acted = (c.side == COMBAT_SIDE_PLAYER)
                        ? player_fn(&c, player_ctx)
                        : combat_ai_action(&c);
        if (acted) {
            // Record the action (before compaction renumbers slots) -- write-only,
            // never affects the result/RNG, so digests stay byte-identical.
            if (out_rec && out_rec->entries && rec_slot >= 0 && rec_slot < COMBAT_SLOTS)
                combat_rec_append(out_rec, &c, rec_side, rec_slot,
                                  rec_px, rec_py, pre_count);
            if (c.unit_id >= 0) {
    ...
    if (out_rec) {
        out_rec->result = c.result;
        // Defender (AI side) survivors at resolution -- observation-only end-state
        // for the recruit optimizer's loss ranking. Reads the post-loop, compacted
        // AI units; never touches the formula or RNG (digests unchanged).
        int as = 0; long ah = 0;
        for (int i = 0; i < COMBAT_SLOTS; i++) {
            const CombatUnit *u = &c.units[COMBAT_SIDE_AI][i];
            if (u->troop_idx < 0 || u->count <= 0) continue;
            as++;
            const TroopDef *t = troop_by_index(u->troop_idx);
            ah += (long)u->count * (t ? t->hit_points : 0);
        }
        out_rec->ai_stacks = as;
        out_rec->ai_hp = ah;
    }
