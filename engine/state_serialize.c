...
static cJSON *build_tile_window(const Game *g, const Map *map, const Fog *fog) {
    ...
    for (int dy = 0; dy < WIN_SIZE; dy++) {
        ...
        for (int dx = 0; dx < WIN_SIZE; dx++) {
            ...
            // Mirrors the walkability check engine/step.c applies on a move.
            bool walkable;
...
cJSON *state_build_snapshot(const Game *g,
    ...
    // A catalog game stores its 0..255 index and re-derives `seed` on load.
    // The raw seed is written only for non-catalog games (tests), and is
    // lossy above 2^53: cJSON numbers are doubles, so a full-width seed does
    // not survive the round-trip. The index always does.
    cJSON_AddBoolToObject(root, "seed_from_catalog", g->seed_from_catalog);
    if (g->seed_from_catalog) {
        cJSON_AddNumberToObject(root, "seed_index", g->seed_index);
    } else {
    ...
    // ---- contract ----
    {
        ...
        cJSON_AddItemToObject(c, "villains_prefought", prefought);
