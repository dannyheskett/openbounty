...
// Walkability rules ported  (src/game.c around line 6507).
//
// Walk mode:
...
InteractResult adventure_handle_interact(const Tile *t, const char *zone) {
    ...
    if (t->interactive == INTERACT_SIGN) {
        ...
        // queue (M3). adventure_handle_interact stays Game-free.
        r.dialog_header[0] = '\0';
    ...
    if (t->interactive == INTERACT_TOWN) {
        // Hand control to main.c, which opens VIEW_TOWN. We bounce back so
    ...
    if (t->interactive == INTERACT_CASTLE_GATE) {
        // Surface the castle id to main.c so it can run visit_castle.
    ...
    if (t->interactive == INTERACT_ARTIFACT) {
        ...
        if (a) {
            // Surface the index; main.c claims it and mutates the tile so
    ...
    if (t->interactive == INTERACT_TREASURE_CHEST) {
        // Surface the chest to main.c for the random loot roll + tile
    ...
    if (t->interactive == INTERACT_ORB) {
        // Surface the orb to main.c for fog reveal and dialog.
    ...
    if (t->interactive == INTERACT_DWELLING_PLAINS ||
        ...
        // main.c can show an informational dialog.
    ...
    if (t->interactive == INTERACT_TELECAVE) {
        ...
        // We carry the placement id so main.c can resolve the pairing.
    ...
    if (t->interactive == INTERACT_NAVMAP) {
        ...
        // continent. We surface the tile so main.c can set the flag,
    ...
    if (t->interactive == INTERACT_FOE) {
        //  / accept_foe (game.c:3574): one
        // tile type. main.c looks up the foe in g->foes[] and decides
