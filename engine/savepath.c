...
void SavePathFlush(void) {
    ...
    // IDBFS keeps the mount in memory and only reaches IndexedDB on an
    // explicit syncfs. Asynchronous by nature and fire-and-forget: the
    // syncfs(true) the HTML shell runs at page load picks up whatever
    // landed. A failure is reported but not fatal -- the save is still
    // valid for the rest of this session.
    //
    // emscripten_run_script rather than EM_ASM: the EM_ASM macros are
    // rejected under -std=c99, which every OpenBounty target uses. Module.FS
    // (not bare FS) because this evaluates in global scope, where only the
    // exported runtime methods are reachable.
    emscripten_run_script(
        "Module.FS.syncfs(false, function (err) {"
        "  if (err) console.warn('OpenBounty: could not persist saves:', err);"
        "});");
#endif
}
...
#if defined(__EMSCRIPTEN__)
    // Web: a fixed path under the IDBFS mount that the HTML shell creates
    // and syncs before main() runs. Deliberately no getenv -- a browser has
    // no environment to read, and this project wires configuration through
...
#if defined(__EMSCRIPTEN__)
    // Web: there is no executable to locate. The pack is preloaded into
    // the virtual filesystem at a fixed path by the build's
    // --preload-file mapping; report the directory holding it.
    const char *webdir = "/assets";
    size_t wn = strlen(webdir);
    if (wn + 1 > out_size) return false;
    memcpy(out, webdir, wn + 1);
    return true;
#else
    char path[1024] = {0};
