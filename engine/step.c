...
bool GameStep(Game *game, Map *map, Fog *fog,
    ...
    if (!flying && nt->interactive != INTERACT_NONE) {
        ...
        if (ir.opened_dialog) {
            ...
            // raise it through the uniform queue (M3).
        ...
        if (ir.entered_town) {
            ...
            // M4: raise the TOWN view through the queue (replace=true: VIEW_TOWN
            // resets the shell stack, formerly views_set). views_open_town set the
            // town context statics above.
            {
        ...
        if (ir.opened_castle) {
            ...
            if (audience) {
                ...
                pending_castle_id[k] = '\0';
                // Siege prompt (OPENKB-SPEC §24.16): name the occupant and ask
                // the SINGLE yes/no question — "Lay Siege". The defending
                // garrison is NOT listed here (that is the separate "gather
                // information" screen, §24.18); listing it under a y/n prompt is
                ...
                pending_castle_id[k] = '\0';
                // Siege prompt (OPENKB-SPEC §24.17): name the villain and ask
                // "Lay Siege" — no garrison listing (see the monster branch
                // above; the listing is the §24.18 gather-info screen). The
