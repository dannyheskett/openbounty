...
When compiled against SDL (the only supported configuration, main.c:21 issues `#error` without `HAVE_LIBSDL`):
...
Each `& 0xFF` mask exists to handle signed `char` extension, without it, a byte 0x80..0xFF would sign-extend on most compilers
...
(Note: cont 1 has Elves at slot 5 again, duplicate of slot 3, likely a typo in source but preserved as-is.)
...
- Computes `troop_id, troop_count = roll_creature(continent, ...)`, fresh roll each visit.
...
- **Fireball** (id 2): `damage_army(game, combat, 25, 2)`, picks target via `pick_target` filter 4 but does not invoke
...
9. **`salt_villains` chain** (6 on cont 0, 4 on 1, 4 on 2, 3 on 3), each villain randomly placed into an unowned castle on its
   continent.
...
- **`game->y = HOME_Y - 2 = 5`**: player starts 2 tiles north
  ...
  adjacent.
- **`game->last_y` initialized to start y, not previous**: `last_x = x = 11; last_y = y = 5`. So foe_follow's first
...
salting variety. Dwellings/foes/artifacts are NOT pre-placed, they're generated from chest tiles at game start.
...
4. **Budget:**
   ...
   - For each player troop slot i with `numbers[i] > 0`:
     `credit += numbers[i] * (troops[troop[i]].recruit_cost / 10)`, one-tenth recruit cost per unit per week.
...
is_external, external_damage, retaliation)` (play.c:1407), **this is the heart of the combat engine**. Reproduced here
...
`unit_ranged_shot(war, side, id, other_side, other_id)` (play.c:1564), wraps `deal_damage(…, is_ranged=1, 0, 0, 0)`. `u.shots--`.
...
unit teleports to (tx, ty) without intermediate steps, flight skips obstacles.
...
- Iteration order is slot 0 → 39 sequentially; if foe N moves
  ...
  position check (`map[fy][fx] != 0x91`) still passes for M, but foe N is now also at 0x91 elsewhere, possibly causing
...
is `(fx + position_x, fy + position_y)` (i.e., `(2*fx, 2*fy)`) which is off-map and corrupts state. In practice this branch
...
- Lists 5 castle-dwelling troops with prices, OR "n/a" when the
  player's total leadership can't fit at least **6** of that troop, i.e. `total_leadership < troops[id].hp * 6`, independent of what's
...
- If `id < FRIENDLY_FOES` (slots 0..4): `accept_foe(game, id)`, these are friendly rolls:
...
Background: `draw_location(2 + DWELLING_HILLCAVE, 0x06, frame)`, hill-cave background with Gnomes animation (openkb hard-codes
...
Dragons (id 24, 0x18) have `troop_numbers[24] = {1, 1, 1, 2}`, only on Impossible? will more than 1 dragon be rolled. Even on
...
- `villain_army_troops[17][5]` and `villain_army_numbers[17][5]`, verified at bounty.c:247 and bounty.c:266.
...
Adds a `compress`-side path (`DOS_LZW(dst, dst_max, src, src_len)`, external function declared but defined elsewhere in tools tree).
