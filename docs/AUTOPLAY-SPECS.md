...
and by a human has been the same world. Where demo mode plays under a player's
...
- **AP-010.** A run has begun in `src/main.c` (the `--autoplay --headless`
  ...
  root-adjacent branches the search reaches by backtracking. The verdict has
...
- **AP-011.** `autoplay/autoplay.c` boot has opened the pack, loaded the
  ...
  demo mode uses. `GameInitSeeded` has expanded the index into `game->seed`
...
- **AP-013.** `autoplay_run` has emitted the ONE authoritative terminal line
  ...
  the most objectives any node reached, universe-normalized (AP-205); it has
...
- **AP-014.** `--validate-pack [LO [HI]]` (default `0..255`, the whole
  ...
  (`ob_diag_set_quiet`) so only the table shows. Exit `0` only when every seed
...
- **AP-015.** THE DAY BUDGET HAS COME ENTIRELY FROM THE CHOSEN DIFFICULTY,
  ...
  economy (commission, astrology, growth, restock) stays phase-locked and the
  run plays the pack's own calendar for that level. An arbitrary day count
  would desynchronize the weekly world, because `passed = start_days -
  days_left` drives week ids, astrology, contract windows, and the growth
  predictor alike. `--autoplay-hero=<class>` has selected the class by pack
  class id (default `knight`, `AUTOPLAY_HERO_CLASS`); a class the pack does
  not define has been a hard error that fails the run rather than silently
...
- **AP-016.** The two verdicts have meant precisely this:
  - **SOLVED**: a full clear has been reached AND COMMITTED. Some node's
    ...
    its root-to-node recording are what the run leaves live. It has been a
    positive result about the seed: this line of play exists and has been
    simulated.
...
- **AP-023.** Replay has been driven by the one dumb applier
  ...
  not describe, so the run has stopped at the first diverging primitive
...
- **AP-024.** `--autoplay` without `--headless` has been the visible mode:
  ...
  combat animator (`src/combat_replay.c`) before the identical resolution is
  applied. On completion the shell has handed the cleared board to the human
  (the same win hand-off demo mode performs).
- **AP-025.** The stamp has been of the PRE-action state, so the recorder has
  had to push BEFORE it mutates the engine: replay checks the fingerprint as a
  precondition, and a stamp taken after the mutation would describe a world
  the replay has not reached yet. Where an action's id is only knowable after
  the mutation (`RA_TAKE_CONTRACT`, whose id is `GameTakeNextContract`'s
...
- **AP-026.** A lost fight has been a temp death, and replay has MIRRORED the
  live side's response to it: the engine teleports the hero home, and the
  live run then runs `exec_temp_death` (zone map reload plus the
  bookkeeping). When `player_io_answer` reports `pres.temp_death`, the
...
- **AP-050.** The planner has been a RE-ENTRANT STEP CORE
  (`autoplay/planner.h`), not a loop: `planner_open` enumerates the objective
  universe and initializes run state, `planner_candidates` runs one cycle
  head (logistics, mover pricing, ordering, the promotion tiers),
  `planner_step` performs ONE atomic attempt, `planner_refresh_done` re-reads
  the done predicates, `planner_done` is the goal test, and `planner_report`
  prints the truthful end-of-line causes. The search (AP-200) has driven these
...
- **AP-057.** `planner_candidates` has ordered the open objectives
  ...
  schedules ahead of crossings, cross-zone ties breaking by zone so one sail
  is followed by that zone's whole slate. Three promotion tiers have then
  ...
  (AP-052) has been applied here: the scepter is offered only when every other
  objective is done.
- **AP-058.** All planner memory for one line of play has ridden in a
  ...
  progressed world re-enumerates a SHRUNK universe (dead foes and consumed
  tiles drop out), so indices shift between opens while the history has to
  follow the objective. `stuck_cycles` has been CALENDAR-FREE (it counts
  opens, not days), so the day budget never shifts a promotion decision.
- **AP-051.** Each attempt (`planner_attempt`) has been do-or-fail under a
  ...
  goal predicate has admitted unless the hero ended MAROONED, and a PREFOUGHT
  siege has kept the world without admitting (a contract-less win resets the
  lord's garrison, real progress a restore would erase; the engine records it
  ...
  - **Pre-gate.** A stand-on objective (consumable / dig) whose end tile has
    ...
    hostile camps within follow range has been a JAW TRAP and skipped this
    cycle (`[STRAND]`).
  - **Post-success probe.** After a success, unbeatable hostiles near the
    hero have counted as WALLS; when the reachable set collapses (below the
    pocket node bound `AUTOPLAY_POCKET_NODES`, `autoplay/exec.h`) the hero is
    inside a closing jaw. Two second chances have run before judging: fight
    the jaw open from INSIDE (the sealer is an objective, and the pocket often
    holds its own funding; a win commits both), else walk out
    (`move_escape_jaw`). Only a failed escape has been a maroon: the success
    rolls back, the nearest sealer is recorded as the objective's BLOCKER,
    and the defer carries the jaw fight's truthful cause (gold / stock when
...
- **AP-052.** One selection ruling has shaped the ordering, the **finale
  ...
  objective is done. The King's-quest-last rule, no exceptions.
- **AP-053.** THE NO-BURN LAW: play has never spent calendar merely waiting.
  ...
  immediately with a typed defer unless the **wait-allowed flag** is armed
  (`autoplay/exec.h exec_set_wait_allowed` / `exec_wait_allowed`). The law
  has been applied at NODE granularity: a node whose candidates all failed to
  ...
  deferring works again. A line still progressing on its own has therefore
  never waited, and all calendar burning has been back-stacked to the point
  where that line proved stuck. THE FUNDED WAIT has ridden the same pass: a
...
- **AP-055.** KEYSTONE PROMOTION AND CYCLE LOGISTICS, two mechanisms that
  ...
  - the ALCOVE KEYSTONE PROMOTION (`planner_candidates`): the alcove unlocks
    ...
    bound has counted OPENS, not calendar days, so the day budget never
    shifts the decision. An alcove that resolves within its natural
  ...
  - transport/stop LOGISTICS (`planner_logistics`, with magic learned and a
    ...
    BULK TIERS once the book widens (the R-C floor always; larger targets at
    the widened-book thresholds documented at their definition site; every
    held charge above the floor is one zero-day crossing, the endgame's
    dominant sink); town gates have topped to the floor once the book affords
    ...
    makes every unstopped step cost the day). Each stocking trip has been
...
- **AP-056.** THE SACRIFICE ESCAPE: when a node has produced no child even
  ...
  stands, one further expansion has taken the game's own escape hatch, a
  deliberate defeat whose temp death teleports the hero home for the price of
  ...
  attempt re-arms it (the bound exists to stop futile escape loops, not to
  ration recoveries that keep buying progress; a maroon-prone geography can
  need one per island). Recorded like any fight and reported on the
  `[PLANNER]` channel; only when no such escape exists has the node exhausted
...
- **AP-054.** The cheapest-first ordering has been refined against one
  ...
  expensive one's only live winner needs, leaving that candidate -- winnable
  at the moment it was first projected -- never projectable again once the
  shared stock is gone. A SCARCE-WINNER CONFLICT TIER has therefore refined
  the selection: before a non-held-win siege best is attempted, each other
  ...
  restock ceiling); when the other candidate's winner DIES under the best's
  draw while the best's own winner survives the reverse draw (live, or
  recoverable at the restock ceiling), the fragile winner has been attempted
  first. Sieges only (foes never bind elite stock), one swap per cycle,
...
- **AP-201.** The frontier has been an AVL multiset (`autoplay/baltree.c`),
  chosen because it gives all three operations a memory-bounded frontier
  needs: pop-min (take the next node), insert (add children and re-insert a
  node with branches left), and pop-max (evict when over the cap). A heap
  gives the first two and not the third.
- **AP-202.** Ordering has been STRICT DEPTH-FIRST. The key has been the
  negated creation sequence (`node_key` = `-seq`), so pop-min returns the
  NEWEST node (the current line's tip) and a re-inserted parent keeps its
  original, older key, so a child and its whole subtree drain before the
  parent's next branch. That has been exact backtracking DFS expressed on the
  tree, and pop-max has evicted the ROOT-MOST pending alternatives, which are
...
- **AP-203.** The DECLARED EXPANSION SET, the bounds that make NOT-SOLVED a
  ...
  - **Branching cap.** `SEARCH_MAX_CHILDREN` child states per node, after
    which its remaining alternatives are trimmed. Failures still fall through
    the WHOLE candidate list (that is the cheapest-first ordering doing its
    job) but successful branching is bounded. The ROOT has been exempt, so
    its alternatives always remain available to a restart.
  - **Beam.** `SEARCH_MAX_LIVE_NODES` live nodes; over that, pop-max evicts
  ...
  - **Stagnation cut.** A descent that has completed nothing for
    ...
    is pruned back to the root, whose next untried candidate opens a fresh
    descent under a different opening objective. Measured on seed 17, where
    low-calendar subtrees fail with `cause=time` about 170 expansions per node
    and plain DFS drains them all before returning.
  ...
  - **Dead leaves.** A child born with the calendar exhausted and work still
    ...
    gate to cost zero days from exactly 0 days left is given up.
...
- **AP-204.** A node has carried the FULL world (`WorldSnapshot`: Game + Map +
  ...
  zone reload) has been measured WRONG: a mid-game fresh stamp is not
...
- **AP-205.** Progress has been UNIVERSE-NORMALIZED. Each node's enumeration
  SHRINKS as objectives complete, so "done within this node's universe" is
  not comparable across nodes. The OPEN count has been the same quantity in
  every view, so true progress is `root total - open`. The goal test has been
  `open == 0`.
- **AP-206.** The recording of a node has been the concatenation of the edge
  ...
  pre-search prefix, so a rebuild reproduces the boot segment too. An edge's
  delta base has been marked BEFORE the candidate pricing pass, because the
  cycle head runs LOGISTICS, which may record prims and move the world: those
  prims belong to the child's edge, or the rebuilt recording would lose the
  stocking trips whose effects its own fingerprints assume.
- **AP-207.** The outcome state has been COMMITTED, never re-simulated: on a
  ...
  replayable recording all describe the same line of play.
- **AP-208.** Because a restore jumps between DIFFERENT lines of play, the
  ...
  same-play rollback deliberately keeps: the mover's nav memo
  ...
  cache returns the same winner the search would have computed.

- **AP-046.** **Vistas have been objectives.** Every `events` entry a zone
  declares (REQ-221b) has been enumerated as a `STEP_VISTA` whose target is
  the trigger tile and whose handle is the event id (`autoplay/goals.c`); it
  has been done when `GameEventFired` says so. The executor (`exec_vista`,
  `autoplay/primitives.c`) has bought any rite the vista requires at a town
  that sells it, then stood on the tile, where the engine plays the scene and
  applies its effects; the prerequisite gate has marked it `PREREQ_MAGIC`
  while the hero cannot hold that rite, and `PREREQ_RELIC` while an artifact
  it asks for is unfound. This has been how a pack gates ground behind
...
- **AP-060.** `autoplay/prereq.c` has encoded only the engine-enforced hard
  ...
  CANDIDATES an attempt runs first: for a monster or villain siege with no
  siege weapons, one BUY-SIEGE candidate (`prereq_make_buy_siege`, resolved
  to the nearest town at execution), and for a gate foe that demands one arm
  ...
  `PREREQ_CONTRACT` (a villain whose contract is not obtainable now),
  ...
  scepter while other work is open, AP-052). A gate foe's demand has
  ...
  MUSTER candidate fetches the arm inside the attempt instead.
- **AP-061.** The villain's active-contract requirement and the
  ...
  take loop has tolerated EMPTIED cycle slots: once catches outnumber
  refills (`GameFulfillContract` advances `max_contract` whether or not a
  refill lands), a rotation step can return no contract while the target
  sits in the NEXT slot, so the loop has kept taking through a full cycle
  instead of stopping at the first empty slot, exactly as a player re-presses
...
- **AP-188.** `exec_fetch` has handled the two ways an arrival fails to fire
  ...
  recruiting one flier from an in-zone dwelling first when none is held) and
...
- **AP-081.** There has been one primitive helper per objective family
  ...
  refuses without charging when the wallet is short), so
  ...
  ends without magic loses the whole sustain-kit candidate class: every
  objective whose only winner needs a combat spell stays unreachable.
...
- **AP-082.** `siege_or_slay` (`autoplay/primitives.c`) has been
  ...
  a failed live verify declines the prompt and earns ONE second chance
...
- **AP-086.** A hostile foe CO-LOCATED with the hero (the foe followed onto
  ...
  pursuit's arrival test is already satisfied, and the round budget would
  ...
  later attempt and turns every strong-army retry into a round-budget spin
...
- **AP-184.** THE WEEK HAS BEEN THE ONLY CALENDAR QUANTUM. Every calendar
  ...
  advances the exact day count to the NEXT week boundary, day-granular at
  boundaries by construction, under THE NO-BURN LAW (AP-053). There has been
  no sub-week rest action, because no plan needs one: every wait the module
  takes has been sized by the weekly economy (income, restock, growth), which
  only moves at those boundaries.
...
- **AP-187.** THE ZERO-DAY STOP RESTOCK LOOP (`autoplay/exec_move.c
  ...
  VISITED town selling Time Stop, stepped in (gate landings sit beside the
  town, `REQ-322`), bought stop charges into the free book less the two slots
  the return refill needs, and gated back to the town-gate seller's own town
...
- **AP-094.** Hostile-foe tiles have been FIGHT-THROUGH edges, not walls
  (foes chase the hero and routinely seal choke points): the A* has priced
  them with a penalty, the drive has stepped on and the responder fought when
  the live prediction wins; a DECLINED fight has added that foe's tile to the
  call's avoid set so re-paths route around the refusal instead of
  re-provoking it.
- **AP-181.** The mover has considered the Bridge adventure spell (`REQ-322`:
  ...
  approach and could defer as unreachable an objective a human reaches
...
- **AP-182.** NO THROUGH-CAVE ROUTING. The A* kernel has carried a telecave
  ...
  the edge has been reachable only when the cave tile is itself a goal: foot
  ...
  measured: any real day saving shifts the committed calendar enough to
  reorder the cheapest-first tour (§5), even with the planner's ordering
  quotes left untouched, so cheap hop-reachable work schedules ahead of the
  villain contracts that fund the whole economy and the run starves. On 30
  ...
  Telecaves have remained non-objectives (they are not added to the
  `PlanStepSet`, AP-040); traversal, where it does occur, has recorded as
...
- **AP-103.** **Once-per-call recovery levers.** A recovery inside one
  ...
  churn within a pass, not the search across calls (R-A.2; the pattern is
...
- **AP-104.** **Declared search bounds.** The third class, and the only one
  ...
  drop (AP-203). They have been permitted because they are declared rather
...
- **AP-110.** A gate cast has never spent the last held gate charge -- that
  ...
  except at the town that sells the gate spell, where a landing refills by
...
- **AP-190.** THE SELF-MAROON GUARD (`autoplay/exec_move.c
  ...
  final target sits inside the pocket (arriving IS the point; the planner's
  maroon probe judges the aftermath). Unknown geometry (a zone that fails to
...
- **AP-130.** The plan catalog has been built once by `autoplay/exec_recruit.c
  ...
  ever builds the lone troop (a pure shooter troop takes no retaliation and
...
- **AP-131.** The search (`exec_recruit.c search_all`) has been a SIM FUNNEL
  ...
  cost bind together (a zero-cost garrison troop subsidizes exactly its
  ...
  (`build_candidate`'s zone mask): a plan gathered where the hero stands
  prices 0, so among winners the cheapest is the easiest to GATHER. The
  ...
  The funnel has made the search tractable on any pack: it collapses a
  candidate universe that is astronomically large under direct simulation
  ...
  hiding between two losing rungs falls through to the typed probe causes
...
- **AP-132.** The leadership lift has been modeled on the engine: each
  ...
  CARRY the charges instead since every crossing's week boundary would burn
  ...
  fight.
- **AP-133.** The win has been verified against the army exactly as it is
  ...
  grown-garrison arithmetic reads, so two grow-weeks sims at different weeks
  can share a verdict. The module has never delivered the hero to a fight
  the simulation does not win.
- **AP-134.** When no engine-legal plan within the search wins,
...
- **AP-136.** Combat-spell (kit) allocation has been two first-class axes of
  ...
  cheaper rung. The combat-mechanic facts that make the sustain choice
...
- **AP-180.** The kit search has enumerated every combat spell in the pack's
  ...
  attrition, not a bigger army: `clone` multiplies an elite troop,
  `resurrect` out-lasts a mega-garrison's damage, `freeze` removes a decisive
  enemy troop for a round, and `turn_undead` / `lightning` land damage where
  `fireball` is wasted.
- **AP-183.** The win predictor and the projection have accounted for the
  ...
  projected winner has been verified against the garrison as it stands on
...
- **AP-138.** THE REALIZE-FAILURE MEMORY (`autoplay/exec_recruit.c`
  `s_excl` / `excl_add` / `excl_has`): a chosen winner whose realize starves
  ...
  recorded at the wallet of a past failure, and a richer attempt deserves the
  full universe (a permanently-poisoned last villain is a lost game).
...
- **AP-141.** The realize (`realize_plan_body`) has run, in order: banked or
  ...
  (`plan_buy_troop`, whose watchdog scales with the ask since bulk plans
...
- **AP-156.** THE APPROACH RESERVE (`autoplay/exec_recruit.c
  ...
  the full `k × raise_cost` at-gate re-arm, since a realize that spends the
  whole wallet strands a committed winner at a sail crossing. The reserve has
...
- **AP-161.** When an attempt has been blocked on money -- the boat fare, a
  ...
  in-zone dwelling when its own upkeep still ate the commission, buying
  before dismissing so the army never empties; and LAST, returned the boat
  rental, but only when the weekly net is still non-positive, since a boat is
  weekly insurance but also the only ride home from a dockless shore, and a
  wait that funds in a week or two never repays stranding the buys'
  crossing),
  ...
  troop is quote credit, and dismissing it after the bisect would invalidate
  the budget the wait was sized against. A mid-move money need (`nav_rent_boat`'s
  fare) has used the no-trim variant (`exec_ensure_gold_no_trim`): a
  committed fight army is never dismissed to afford a boat fare, so it has
...
- **AP-162.** THE FUNDED-WAIT MECHANICS (`exec_recruit`, wait-allowed pass
  ...
  unbounded wallet has proved any winner exists at all; a bisection has then
  found the smallest budget that buys one; the funded target has added a
  fixed working-capital margin (the divisor at its definition site;
  charge-trip ratchets, transport refills, and the lift's own consumption are
  overhead the candidate price does not carry). THE GROWTH-AWARE WAIT: the
  bisect prices TODAY's garrison, but the wait's own weeks grow it (AP-183),
  so the need has been re-probed at the garrison as it stands when the
  wallet arrives, and a RECEDING target (growth adds more budget than the
  whole wait delivers) has been refused outright so cheaper candidates run
  ...
  sampler off the proven winner; the surplus stays for the realize's working
  capital), falling back to the full wallet and finally to the existence
  probe's own winner rather than deferring a fight the wait just funded.
...
- **AP-170.** All module diagnostics have been permanent, CLI-gated, and
  ...
  diagnostic has been emitted only when the flag is set, and no game state
...
- **AP-174.** DEFAULT OUTPUT HAS BEEN A SUMMARY, not a trace. A run without
  ...
  maintained -- they are cheap and they are how performance questions get
...
- **AP-171.** The channels, all gated by `--verbose` and each emitting its own
  ...
  - the recruiter trace (`autoplay/exec_recruit.c`), `[RECRUIT]`
    ...
    dropped; a line per DROPPED CANDIDATE measures 3.3M of 3.3M verbose lines
...
- **AP-175.** THE `[UNMET]` REPORT has named, for each objective the run did
  ...
  representative objective, so the report is bounded by the number of
...
- **AP-173.** A behaviour-inert diagnostic has been allowed to read the very
  ...
  (`CombatTurnRecord.ai_stacks` / `ai_hp`) a candidate fight already
  produced, because such a read adds no engine call and changes no state. A
