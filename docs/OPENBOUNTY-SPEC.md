...
as built. This single specification merges what were previously two documents
(`GAMEPLAY-SPEC.md`, the game-design/rules spec, and an earlier
`OPENBOUNTY-SPEC.md` implementation spec) into one authoritative source.
...
World Computing). It descends from **OpenKB** (an earlier SDL2 reimplementation)
and deliberately diverges from it in several architectural respects (§1.10);
gameplay-significant constants and formulas match OpenKB except where a
deviation is explicitly flagged (§34).
...
- **REQ-090.** OpenBounty descends from OpenKB but diverges deliberately. These
  ...
  | Saves | 20,421-byte binary | JSON (version 7) |
...
- **REQ-111.** Tunable constants that *define gameplay* (day/week lengths,
  costs, contract cycle length, difficulty table) have **not** been compile-time
  — they live in `game.json` and are read through `g->res`. Only storage caps
...
- **REQ-142.** Declaration order and meaning:
  ...
  | `version` | `int` | Always `SAVE_VERSION` (7) at runtime |
  | `seed` | `uint64_t` | RNG seed for this game |
...
- **REQ-166.** The new-game seed has been `time(NULL) XOR hash(name) XOR
  class_index`, except when `--seed N` has been supplied, in which case `N` is
  used verbatim.
...
- **REQ-185.** Given the same seed and the same sequence of player inputs, the
...
- **REQ-186.** All randomised game-start placements (zones, villains, scepter,
  ...
  from `seed`, so loading a save restores identical placements.
...
- **REQ-271.** Per-stack morale: a single non-empty stack is High; otherwise
  each other non-empty stack is consulted on the chart and results counted —
  any `L` → Low; all `H` (≥1) → High; else Normal.
...
- **REQ-351.** `GameRollChest(zone, x, y)` (`engine/game.c`) rolls `chance =
  (chest_rand(g,x,y,1) % 100) + 1` and walks cumulative thresholds in order —
  gold → commission → spell_power → max_spells → new_spell → empty — taking the
...
- **REQ-410.** Saves have been **JSON, version 7** (`SAVE_VERSION` in
...
- **REQ-414.** The save schema mirrors the `Game` struct field-for-field (§4),
  so serialization round-trips without loss. A save-format change bumps
  `SAVE_VERSION` and updates/replaces the golden fixture
...
- **REQ-432.** Two dialog flavors: `open_dialog(header, body)` (`src/ui.c`) —
  a bottom-frame box dismissed by any key, supporting paginated body (split on
  form-feed `\f`); and the blocking prompts (`src/prompt.c`) —
  `prompt_yes_no_open`, `prompt_numeric_open`, `prompt_text_input_open` — whose
...
- **REQ-480.** Parsed in `src/main.c` (early-exit modes in
  ...
  `--pack <name|path>`, `--save-dir <dir>`, `--seed N`, `--movie [<path>]`,
  `--extract`, `--out-dir <dir>` (modifier for `--extract`), `--pack-dir <src>
  <dst>`. Normal play takes no flags.
...
- **REQ-482.** Three release platforms (`docs/RELEASE-PROCESS.md`): Linux
  x86_64 (tar.gz), Windows x86_64 + i686 (zip, single static .exe, no DLLs),
  macOS universal (zip, arm64 + x86_64, ad-hoc signed). Releases are sequential
  build numbers (`v1`, `v2`, …); the build number is embedded at compile time
  and exposed via `--version`.
...
## 38. Known deviations from OpenKB and incomplete features

These are the standing differences from OpenKB and the genuinely unimplemented
items. (Combat and combat spells, which an earlier draft of this spec described
as stubbed, are **fully implemented** — see §25; that stub description was
stale and has been removed.)

### 38.1 Architectural divergences (no gameplay change)

- **REQ-520.** raylib instead of SDL2; JSON saves instead of the OpenKB
