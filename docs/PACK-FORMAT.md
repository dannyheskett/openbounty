...
| `tuning`      | object   | Spell multipliers, search cost. |
...
`%COUNT%`). Tokens are documented per-string in `src/resources.h` next
to each field.
...
1. Verify it loads in a development build (drop the directory under
   `~/.local/share/openbounty/packs/` on Linux, `%APPDATA%\OpenBounty\packs\`
   on Windows).
2. Zip the pack root into `<pack_id>.openbounty` (the file extension is
   what tells the engine to treat it as a pack).
3. Drop the `.openbounty` file into the user packs directory.

The engine's pack picker discovers both loose directories and
`.openbounty` archives at startup.
