...
# Headers too: the install dirs are gitignored, so this script is the only
# thing that puts raylib.h where the Makefile's -I expects it.
cp "$RAYLIB_SRC_DIR/src/raylib.h" \
   "$RAYLIB_SRC_DIR/src/raymath.h" \
   "$RAYLIB_SRC_DIR/src/rlgl.h" "$RAYLIB_INSTALL_DIR/include/"
